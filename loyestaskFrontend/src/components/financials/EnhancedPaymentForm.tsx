...
export default function PaymentForm({ payment, onSuccess, onCancel }: PaymentFormProps) {
    ...
    const createPaymentMutation = useMutation({
        mutationFn: async (data: CreatePaymentForm) => {
            ...
            if (selectedFiles.length > 0) {
                ...
                return PaymentAPI.create({ ...data, attachments: uploadedPaths })
            }
    ...
    const updatePaymentMutation = useMutation({
        mutationFn: async ({ id, data }: { id: string; data: Partial<CreatePaymentForm> }) => {
            // Si hay archivos nuevos, subirlos
            if (selectedFiles.length > 0) {
                const uploadedPaths = await uploadFiles(selectedFiles)
                return PaymentAPI.update(id, { 
                    ...data, 
                    attachments: [...uploadedFiles, ...uploadedPaths] 
                })
            }
    ...
    const uploadFiles = async (files: File[]): Promise<string[]> => {
        ...
        try {
            ...
            return result.files.map((file: { filePath: string }) => file.filePath)
        } catch (error) {
    ...
    const onSubmit = (data: CreatePaymentForm) => {
        const paymentData = {
            ...
            paymentDate: data.paymentDate // Ya es string del input type="date"
        }
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const validTypes = ['image/*', 'application/pdf', '.pdf', '.jpg', '.jpeg', '.png']
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
                ...
                {/* Archivos ya subidos (en edición) */}
